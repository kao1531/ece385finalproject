...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	status = XSpi_Transfer(&SpiInstance, arr, NULL, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	status = XSpi_Transfer(&SpiInstance, arr, NULL, nbytes+1);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE arr[2] = {reg, 0};
	BYTE recBuf[2] = {0, 0};
	status = XSpi_Transfer(&SpiInstance, arr, recBuf, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	status = XSpi_Transfer(&SpiInstance, arr, brr, nbytes+1);
